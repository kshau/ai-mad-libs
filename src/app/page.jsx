...
export default function Home() {
	...
	const getFilledMadLib = () => {
		const madLibContentWords = madLibContent.match(
			/(\[[^\]]*\])|[\w]+(?:'[\w]+)?|[.,\/#!$%\^&\*;:{}=\-_~()]/g
		);
		...
		madLibContentWords.forEach((word) => {
			if (word.includes("[")) {
				newMadLibContentWords.push(
					`__${filledBlanks[filledBlanksIdx]}`
				);
				filledBlanksIdx++;
			} else {
			...
			<div className="min-w-[50rem] max-w-[70rem] mt-20">
				...
				{madLibContent == null ? (
					<Card variant="outline">
						<CardHeader>
							<CardTitle>Write a Topic</CardTitle>
						</CardHeader>
						<CardContent>
							<form>
								<Input
						...
						{filledBlanks == null ? (
							...
							<Card>
								...
								<CardContent className="flex flex-col">
									...
									<Button
										className="flex flex-row gap-x-2 mt-4 w-fit"
										onClick={() => {
											location.reload();
										}}
									>
